    ...
    Board() {
        ...
        for (int xSize=1; xSize < board_size + 1; ++xSize)
        {
            ...
            for(int ySize=1; ySize < board_size + 1; ++ySize) {
                ...
                board[xSize][ySize] = valueOfPosition;
            }
        }
    }
};    
...
int main() {
    ...
    Board thisBoard;
    // Continue playing until the player quits
    int player=0; // variable to update player, must be 1 or 2
    while(thisBoard.getGameState()) {     
        // display board
        thisBoard.show();
        
        // Prepare round for player
        player = thisBoard.switchPlayer(player);
        thisBoard.setCurrentPlayer(player); // Updates user listed
        thisBoard.setPlayerSymbol(player); // Change to X for player 1, or O for player 2
        playerSymbol = thisBoard.getPlayerSymbol();
